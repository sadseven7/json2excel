...
import com.xiaojian.json2excel.domain.Physical;
...
@SpringBootTest
public class json2execl {

    public static int i_Inquiry = 0;
    public static int i_Screening = 0;
    public static int i_CycleCheck = 0;
    public static int i_Physical = 0;
        ...
        public void J2E() throws IOException {
             ...
             */
            String jsonStr = "";
//            FileReader reader = new FileReader("D:\\test\\in.json");
            InputStreamReader reader = new InputStreamReader(new FileInputStream(new File("D:\\test\\in.json")), "UTF-8");
            ...
            List<SXSSFRow> titleRowList = new ArrayList<>();
...
//                    subjectInfo = MapperUtils.json2pojoByTree(datas.get(i).toString(),"SubjectInfo",SubjectInfo.class);
                    ...
                    if(flag){
                        for (Map.Entry e : datas.getJSONObject(0).entrySet()) {
                            ...
                            if (!name.equals("SubjectInfo")){
                                ...
                                if(name.equals("Screening")||name.equals("Inquiry")||name.equals("CycleCheck")||name.equals("Physical") ){
                                    SXSSFSheet sheet1 = workbook.createSheet(name.toString()+"_Details");// 创建一个Excel的Sheet
                                    SXSSFRow titleRow1 = sheet1.createRow(0);//创建表头
                                    detailsMap.put(name.toString()+"_Details",sheet1);
                                    detailsMap.put(name.toString()+"_Details_T",titleRow1);
                                }

                                sheetList.add(sheet0);
                                titleRowList.add(titleRow0);
                    ...
                    for (int k=0 ;k<nameList.size();k++){
                        if (nameList.get(k).equals("Summary")){
                            buildSheetO(sheetList.get(k),titleRowList.get(k),nameList.get(k),datas,Code,i,istitleList);
                        }else {
                            buildSheetA(sheetList.get(k),titleRowList.get(k),detailsMap,nameList.get(k),datas,Code,i,istitleList);
                        }
                    }
//                    buildSheet(sheet_Inquiry,titleRow_Inquiry,"Inquiry",datas,Code,i,istitleList);
                ...
                runing++;
                if (runing>=1000 && runing%1000==0){
                    System.out.println("当前进度："+runing+"/"+total);
                }
                if (runing == total){
                    System.out.println("******完成全部："+total);
                }
            }
         ...
         */
        @SneakyThrows
        public void buildSheetA( SXSSFSheet sheet,SXSSFRow titleRow,Map detailsMap, String sheetName, JSONArray datas, String Code, int i, List istitleList){
...
//                System.out.println(json.toString());
                ...
                for (Map.Entry entry : json.entrySet()) {
                    ...
                    if (!istitleList.contains(sheetName)) {
                        //写入标题
//                        titleRow = sheet.createRow(0);//创建表头
                    ...
                    if (velue != null) {
                        if (key.toString().equals("Details")) {
                            ...
                            SXSSFRow titleRow0 = (SXSSFRow)detailsMap.get(sheetName_Details+"_T");
//                            System.out.println(json.getJSONArray("Details"));

                            for(int k = 0 ;k<json.getJSONArray("Details").size();k++){
                                buildDetails_Sheet( sheet0, titleRow0, sheetName_Details, json.getJSONArray("Details"), Code, k, istitleList);
                                if(sheetName.equals("Inquiry")){
                                    i_Inquiry++;
                                }else if(sheetName.equals("Screening")){
                                    i_Screening++;
                                }else if(sheetName.equals("CycleCheck")){
                                    i_CycleCheck++;
                                }else if(sheetName.equals("Physical")){
                                    i_Physical++;
                                }
                            }
                        } else {
         ...
         */
        @SneakyThrows
        public void buildSheetO( SXSSFSheet sheet, SXSSFRow titleRow, String sheetName, JSONArray datas, String Code, int i, List istitleList){
...
//                    SXSSFRow titleRow = sheet.createRow(0);//创建表头
                ...
                //写入内容
                if (velue != null) {
                    if (key.toString().equals("Details") && sheetName.equals("Inquiry")) {
                        row.createCell(j + 1).setCellValue("见" + sheetName + "_Details表");
                    } else if (key.toString().equals("Details") && sheetName.equals("Screening")) {
                        row.createCell(j + 1).setCellValue("见" + sheetName + "_Details表");
                    } else {
                        row.createCell(j + 1).setCellValue(velue.toString());
                    }
...
//                    System.out.println("***********2");
         ...
         */
        @SneakyThrows
        public void buildDetails_Sheet( SXSSFSheet sheet, SXSSFRow titleRow,String sheetName, JSONArray datas, String Code, int i, List istitleList) {
            //创建行
            SXSSFRow row = null;
            if(sheetName.equals("Inquiry_Details")){
                row = sheet.createRow(i_Inquiry + 1);
            }else if(sheetName.equals("Screening_Details")){
                row = sheet.createRow(i_Screening + 1);
            }else if(sheetName.equals("CycleCheck_Details")){
                row = sheet.createRow(i_CycleCheck + 1);
            }else if(sheetName.equals("Physical_Details")){
                row = sheet.createRow(i_Physical + 1);
            }else {
                err.println("其他类型_Details："+sheetName);
            }
